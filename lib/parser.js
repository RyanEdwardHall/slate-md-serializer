...
var hashtag = new RegExp((0, _hashtagRegex["default"])().source.replace(/^/, '^(').replace(/$/, ')'));
...
block.blockquote = replace(block.blockquote)('list', block.list)('hr', block.hr)('heading', block.heading)('def', block.def)();
  ...
  while (src) {
    ...
    if (cap = this.rules.list.exec(src)) {
      ...
      for (; i < l; i++) {
        ...
        var checked = todo ? Boolean(item.match(/^ *(\[x\])/)) : undefined;
        space = item.length;
...
Renderer.prototype.code = function (childNode, language) {
  ...
  return {
    ...
    type: 'code',
    data: data,
    nodes: this.groupTextInLeaves(childNode).map(function (node) {
...
Renderer.prototype.underlined = function (childNode) {
  ...
  return childNode.map(function (node) {
    if (node.type === 'link') {
      _this3.underlined(node.nodes[0].leaves);
...
Renderer.prototype.strong = function (childNode) {
  ...
  return childNode.map(function (node) {
    if (node.type === 'link') {
      _this4.underlined(node.nodes[0].leaves);
...
Renderer.prototype.em = function (childNode) {
  ...
  return childNode.map(function (node) {
    if (node.type === 'link') {
      _this5.underlined(node.nodes[0].leaves);
...
Renderer.prototype.del = function (childNode) {
  ...
  return childNode.map(function (node) {
    if (node.type === 'link') {
      _this6.underlined(node.nodes[0].leaves);
...
Renderer.prototype.ins = function (childNode) {
  ...
  return childNode.map(function (node) {
    if (node.type === 'link') {
      _this7.underlined(node.nodes[0].leaves);
...
Renderer.prototype.userMention = function (userID) {
  return {
    ...
    nodes: [{
      object: 'text',
      text: "@".concat(this.options.usersContext[userID]),
      marks: [{
        type: 'bold'
      }]
    }]
